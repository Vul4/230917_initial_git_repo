...
knitr::purl('graphics.Rmd', documentation = F)
```

```{r echo=FALSE, purl=F}
xfun::embed_file('graphics.Rmd')
```

```{r echo=FALSE, purl=F}
xfun::embed_file('graphics.R')
```

```{r echo=FALSE, purl=F}
xfun::embed_file('Data/FAO_grains_NA.csv')
```

```{r echo=FALSE, purl=F}
xfun::embed_file('Data/Income_education.csv')
```
...
The most intuitive plot function in R is `plot` which generates both points and line plots. For example, to plot `Petal.Length`,  `Petal.Width`, or `Petal.Length` vs `Petal.Width` we can do:
...
The latest takes two arguments: `Petal.Length` and `Petal.Width` which is to be read as plotting the variable `Petal.Length` as a function of `Petal.Width`. The **argument** `data` simply indicates the plot function from which data frame to extract the variables  Another way to call this command is to type:

```{r,  eval=F}
plot(iris$Petal.Length ~ iris$Petal.Width) # using the $ operator
```

Of course, the `plot` function can take on many other arguments to help customize the default plot options. For example, we may want to change the axis labels to something more descriptive than the simple variable name and give a title to our plot:
...
We can change the symbol type using argument `pch` (**p**lotting **ch**aracter, see `?pch` for options). `pch = 19` will produce a solid circle. The argument `cex` will control his size (**c**haracter **ex**pansion). We can also set its **col**ors to be 90% transparent (10% opaque) using `col = rgb (0,0,0,0.10)`. The `rgb` function defines the intensity for each of the display primary colors (on a scale of 0 to 1). The primary colors are **red**, **green** and **blue**. The forth value is optional and provides the fraction opaqueness (or transparency) with 1 being completely opaque. 
...
Now it is time to combine this with what we learned previously on conditional selection. How to add different colors for the three different species. Do you remember the `ifelse`?
...
We can simply insert this color vector in the `col` argument. In this case, the opacity can be control with the `alpha` function from the package `scales`. Form this example, you can guess the argument `col` can accept a vector of **character** with just the **name of color** instead of a `rgb` code. Conveniently, you just need to call the color you want.
...
Now, a legend must be added  to the plot. Check below the conversion of the color into a factor, and the extraction of its levels in the `col` argument of the `legend`.
...
The argument `las=1` will rotate y-axis labels. This is often a requirement for publication of quality plots. 
...
It looks like the variable `Sepal.Width` is poorly informative in discriminating the three species.

### Types

#### Panel plot

The function `pairs` allows a quick examination of the relationship among variables : that's called a **scatterplot matrix**. 
...
A conversion to a **line** plot is easily made by specifying the `type` of plot.
Let's consider a new data frame reporting the number of iris blossoming in a garden in Alaska and in a garden in Canada from 2010 to 2019
...
The plot is a static object meaning that we need to define the axes limits before calling the original plot function. Both axes limits can be set using the `xlim` and `ylim` parameters. We don’t need to set the x-axis range since both variables cover the same year range. We will therefore only focus on the y-axis limits. We can grab both the minimum and maximum values for the variables `count.alaska` and `count.canada` using the `range` function, then pass this range as the limits using the `ylim` argument within the plot function.
...
Note: point plots from different variables can also be combined into a single plot using the `points` function instead of the `lines` function. Let's get back to our iris dataset and build our plot sequentially for two species only.
...
The documentation states that the breaks value “**is a suggestion only as the breakpoints will be set to `pretty` values**”. `pretty` actually refers to a function that rounds values to powers of 1, 2 or 5 times a power of 10.
...
**QQplot* is an important plot called by the function `qqnorm` to examine the normality of a variable. A `qqline` can be added to represent a “theoretical”, by default normal, quantile-quantile plot.  
...
<p class="comment">
**Practice 4.1** Using `rairuoho` dataset, look at the relationship between the length of the plant at `day3` and the length at `day7`: create a scatterplot. Examine if the length at `day7` is approaching a normal distribution. You will make a use of `hist`, `density` plot and `qqnorm`. How does the`treatment` affect the length at day7? Make a `boxplot` to visualize it. Compare the length of the Easter Grass among days using function `pairs` (scatterplot matrix). Conclude.
</p>
...
The latest will apply to all the following plots and until the graphical parameters  are reinitialize using `dev.off()`
...
Extra elements to add in a plot can be entered seperately:
...
- Already mentioned: `line`, `points`, `legend`, etc.


# Exporting

You might need to export your plots as standalone image files for publications. R can export to many different **raster** image file formats such as jpg, png, gif and tiff, and several **vector** file formats such as PostScript, svg and PDF. You can specify the image resolution (in dpi), the image height and width, and the size of the margins.
...
To save the same plot to a pdf file format, simply substitute `tiff` with `pdf` and adjust the relevant parameters:
...
<p class="comment">
**Practice 4.2** Export the previous `rairuoho` scatterplot. Tune it as much  informative as you think it needs while customizing graphical options. Make the y-axis starting at **exactly** 0 and ending at 130. Save your plot as a `pdf` as it should be prepared to submit a publication!
</p>

```{r, code_folding = 'Show Me The Solution',  eval=FALSE}
...
Note the importance of having a table shaped in **long format**. The density plots are conditioned on **species** which requires that  species be treated as different values of a same variable (i.e. column `Species` in `iris`). The above plot could not be generated if the various **species**  were treated as separate variables (i.e. columns).

You can also compare the distributions to a normal distribution using `qqmath` (i.e. a Q-Q plot):
...
**Boxplots** are another popular way to view the distribution of values. In the following example, we create boxplots of Petal Length as a function of a fake variable `variety` (`Petal.Length ~  variety`) created for the purpose of this demo. We condition this comparison using species information (`|Species`) 
...
The `lattice` package allows logically to plot several variables on a same plot. For example, we can plot `Sepal.Length + Sepal.Width` vs `Petal.Length + Petal.Width` for each `Species` on a same plot:
...
One more example for illustrating line plots using `lattice` and after `order` data `iris` by `Petal.Length`.
...
As the `par` function mentioned earlier, Various elements of a trellis plot can be customized by passing the graphic parameters to the `trellis.par.set` function. To get the list of graphic parameters and their values call:

```{r,  eval=F}
trellis.par.get()
```

It gives you an idea about how many parameters can be customized. Those defaults can be store in an object to further restore them. Many tutorials are available online on the `lattice` package in order to customize your plots as you need. However, the `lattice` package is not always easy to use, and inspire the main author of another package `ggplot` that now becomes the "reference" in matter of graphics in R.


# `ggplot2` 

This is a wide topic which could be the entire course on its own. With the raise of `tidyverse`, the package `ggplot2` has now imposed itself in the **art** of data visualization in R. Below is a very short introduction of this package, and full tutorial can be found online. Here is a recent tutorial from Manny Gimond (2020) is used to demonstrate the power of this package. You can review this section on your own as too many details will not be covered here.


## Basic grammar

The `ggplot2` package is designed around the idea that statistical graphics can be decomposed into a formal system of grammatical rules. The `ggplot2` learning curve is steep but once you  mastered you will have a great control over graphical design. For an up-to-date list of `ggplot2` functions, you may want to refer to ggplot2’s [website](https://ggplot2.tidyverse.org/reference/).
...
Today, we will use the following data sets (examine objects created) : 
...
(2) `dat2`: [income/education census data] (https://www.socialexplorer.com/data/ACS2016/metadata/?ds=ACS16&table=B20004) for the US and its long version (only for advance use later).  
...
From a ggplot **grammatical perspective**, a scientific graph is the conversion of *data* to **aesthetic** attributes and **geometric** objects. This is an important concept to grasp since it underlies the construction of all graphics in `ggplot2`.
...
A violin plot is a symmetrical version of a density plot which provides even greater detail of a sample’s distribution than a boxplot.
...
Note that you can replace `fct_reorder` from the `forcats` packages with the `base` function `reorder` mentioned ealier in the class.
...
Logically, geometries can be  layered. For example, to overlay a linear regression line to the data we can add a `geom_smooth` layer:
...
The `geom_smooth` can be used to fit other *lines* such as a loess line:
...
Little trick for data visualization with `ggplot2` summarized [here](https://rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf). You will have to spend quite a lot of time to get all the functionality off the `ggplot2`. Happily, many documentations are available online . Check the [R gallery](https://www.r-graph-gallery.com/).
...
In the last two chunks, we *filled* the bars with colors (note the use of functions with the string `_fill_`). When assigning color to point or line symbols, use the function with the `_colour_` string. For example:
...
The functionality of the `lattice` package are also available in `ggplot2`. It is called *faceting*
